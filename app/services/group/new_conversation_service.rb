class Group::NewConversationService
  ...
  def call
    ...
    if new_group_conversation.save
      arr_of_user_ids = [creator.id, pchat_opposed_user.id, new_user_to_chat.id]
      ...
      create_initial_message(creator, arr_of_user_ids, new_group_conversation)
  ...
  def create_initial_message(creator, arr_of_user_ids, new_group_conversation)
    message = Group::Message.create(
      user_id: creator.id,
      content: 'Conversation created by ' + creator.name,
      added_new_users: arr_of_user_ids,
